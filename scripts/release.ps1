...
.DESCRIPTION
  This script updates the package version in `pyproject.toml` and `laketrace/__init__.py`,
...
.NOTES
  - Recommended to run in PowerShell Core (pwsh).
...
$initFile = Join-Path $repoRoot 'laketrace\__init__.py'
...
Write-Info "Updating laketrace/__init__.py..."
...
if (Test-Path $readme) {
  ...
  $r = $r -replace [regex]::Escape("LakeTrace v$oldVersion"), "LakeTrace v$NewVersion"
...
if ($Build) {
  ...
  if (Test-Path $buildScriptPS) {
    ...
    Write-Warn "No build script found: checked scripts\build_pypi.ps1"
  }
}
